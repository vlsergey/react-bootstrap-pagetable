...
describe('reactRouter', () => describe('fetchArgsToUrlParams', () => {
  ...
  it('Correctly serialized second page as page=1', () => {
    assert.equal('page=2&size=10', fetchArgsToUrlParams(itemModel, undefined, '', {page: 1, size: 10}).toString());
    assert.equal('myPrefixpage=2&myPrefixsize=10', fetchArgsToUrlParams(itemModel, 'myPrefix', '', {page: 1, size: 10}).toString());
    assert.equal('page=2&size=10', fetchArgsToUrlParams(itemModel, undefined, 'page=42', {page: 1, size: 10}).toString());
    assert.equal('foo=bar&page=2&size=10', fetchArgsToUrlParams(itemModel, undefined, 'foo=bar', {page: 1, size: 10}).toString());
